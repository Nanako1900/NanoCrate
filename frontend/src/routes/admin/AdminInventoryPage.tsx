...
const LEDGER_TONE: Record<LedgerKind, BadgeTone> = { restock: 'in', reserve: 'low', commit: 'neutral', release: 'accent' };
          ...
          <table className="w-full min-w-[32rem] text-sm">
            ...
            <thead>
              <tr className="border-b border-line bg-surface-sunken">
                ...
                <th scope="col" className="label-mono px-3 py-2 text-right">Δ</th>
                <th scope="col" className="label-mono px-3 py-2 text-right">Available</th>
                <th scope="col" className="label-mono px-3 py-2 text-left">Note</th>
              </tr>
            </thead>
            <tbody>
              {data.map((entry) => (
                <tr key={entry.id} className="border-b border-line/60 last:border-0">
                  ...
                  <td className={cn('px-3 py-2 text-right font-mono tabular-nums', entry.delta >= 0 ? 'text-stock-in-ink' : 'text-stock-out-ink')}>
                    {entry.delta >= 0 ? `+${entry.delta}` : entry.delta}
                  </td>
                  <td className="px-3 py-2 text-right font-mono tabular-nums text-ink">{entry.available_after}</td>
                  <td className="px-3 py-2 text-ink-faint">
                    {entry.note}
                    {entry.order_id && <span className="ml-1 font-mono text-2xs">({entry.order_id})</span>}
                  </td>
                </tr>
              ))}
            </tbody>
...
export function AdminInventoryPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useInventory(lowOnly);
  ...
  const columns: Column<InventoryRow>[] = [
    ...
    { key: 'available', header: 'Available', align: 'right', cell: (r) => <span className={cn('font-mono tabular-nums', r.available === 0 ? 'text-stock-out-ink' : r.is_low ? 'text-stock-low-ink' : 'text-ink')}>{r.available}</span> },
    ...
    { key: 'on_hand', header: 'On hand', align: 'right', hideBelow: 'md', cell: (r) => <span className="font-mono tabular-nums text-ink-soft">{r.on_hand}</span> },
    {
      key: 'status',
      header: 'Status',
      cell: (r) =>
        r.available === 0 ? <Badge tone="out" mono>out</Badge> : r.is_low ? <Badge tone="low" mono>low</Badge> : <Badge tone="in" mono>ok</Badge>,
    },
  ];
